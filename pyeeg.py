...
def bin_power(X,Band,Fs):
	...
	for Freq_Index in range(0,int(len(Band)-1)):
		Freq = Band[Freq_Index]
		Next_Freq = Band[Freq_Index+1]
		tmp_low = Freq/Fs*len(X)
		tmp_upp = Next_Freq/Fs*len(X)
		low = int(math.floor(tmp_low))
		upp = int(math.floor(tmp_upp))
	Power[Freq_Index] = sum(C[range(low,upp)])
